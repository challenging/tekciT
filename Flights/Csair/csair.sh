    ...
    do
        ...
        do
            ...
            if [ ${isDone} -eq 0 ]; then
                ...
                /usr/local/bin/scrapy crawl ${spiderName} -a fromCity="${fromCity}" -a fromCityCode=${fromCityCode} -a toCity="${toCity}" -a toCityCode=${toCityCode} -a dateStart=${dateStart} -a dateEnd=${dateEnd}
                ret=$?
                if [ ${ret} -eq 0 ]; then
